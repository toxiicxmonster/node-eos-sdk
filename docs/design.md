...
Instructions for a native Node addon wrapping the EOS C SDK, so a game can run
cross-store multiplayer between Steam and Epic Games Store players.

Written 2026-09-20. This is the design note the package was built from, kept
because the reasoning behind the scope and the tick model is worth more than a
summary of it.
...
The netcode this was written for is deterministic lockstep: every peer
simulates the whole world and only orders cross the wire. That part was
...
- Only workable if the addon is loaded in the renderer, which it should not be:
  ...
  (see the preload script), and that posture is worth keeping.
...
frame rate. The lockstep turn is 200 ms (six ticks per turn at 30 Hz), so a 50 ms
...
A "socket" here is just a named channel, e.g. `{ SocketName: "mygame" }`. Both
...
the existing narrow `contextBridge` in the preload script, which currently
...
The game side is already prepared. A transport module defines the seam:
...
`Lockstep` takes `send` and is fed by `receive`, and
...
An EOS transport is therefore a new file, `transport-eos.js`,
implementing the same interface against the `window.desktop.net` bridge. Nothing
in the lockstep, command, checksum or lobby UI modules changes.
...
Reuse an existing lockstep load generator, if you have one. A good one drives two
